...
| catalyst (Catalyst) | 종목에 묶인 시간축 이벤트. v1 범위: **실적발표일**(KR/US) + **DART 단일판매·공급계약체결**(KR). 소스 KR=DART 공시검색, US=yfinance 실적일 (기존 의존). 후보 카드에 `다가오는 catalyst: 실적 D-12`, `최근 catalyst: 공급계약 5/28` 형태로 표기. 락업·지수편입·배당락은 defer. |
