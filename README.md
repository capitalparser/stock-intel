...
여기에 더해, 시장 국면과 종목 후보를 한 화면에서 보는 **개인 투자 상황판(lens dashboard)** 을 제공합니다. 미국·한국 **듀얼 시장 국면**(percentile 기반)부터 한·미 후보별 **매력도·감사인 독립성 차단·수급/공매도·catalyst·밸류에이션 기대치·정책 테마**까지 같은 층위의 판단 관점으로 묶어 보여줍니다.

![개인 투자 상황판](docs/assets/dashboard.png)
...
RECOMMENDATION_ERROR_COOLDOWN_PATH=./state/recommendation_symbol_errors.json
RECOMMENDATION_ERROR_COOLDOWN_SECONDS=21600
```
...
추천 스캔에서 TradingView가 열지 못한 심볼은 기본 6시간 동안 추천 후보 풀에서 잠시 제외합니다. watchlist 원본은 수정하지 않고, 같은 오류 심볼이 반복적으로 앞단을 막지 않도록 운영용 쿨다운만 적용합니다.
...
- **듀얼 시장 국면** — 미국 국면과 한국 국면을 각각 독립 4-state(`risk-on`/`conditional`/`fragile rally`/`risk-off`)로 판정합니다. 절대 상수가 아니라 **자기 1년 분포 대비 percentile**(+ 구조적 위험 절대 가드레일: VIX 20·USDKRW 1450 등)로 평가해 "지금이 평소 대비 어디인지"를 객관적으로 봅니다. 국면 이력을 저장해 **국면 전이**(어제→오늘)와 연속일(streak), 변동이 큰 **whipsaw**까지 표시합니다. 한국 국면은 KOSDAQ/KOSPI 상대강도·원화·외국인 시각(EWY)·실현변동성으로 구성합니다.
- **한·미 후보 universe** — 미국은 Market Insights 큐레이션, 한국은 섹터 동적 스크린(반도체/전력기기/조선·방산/금융/AI 등 40종목)을 합집합으로 둡니다. 미국 편향 없이 국장 후보가 KR 국면의 판정을 받습니다.
- **봇 시그니처(이 봇만의 차별점)** — 후보 카드에 **감사인 독립성 차단**(직무상 매입을 피해야 하는 감사인 보유 종목은 `🚫 차단` 배지 + 랭킹 강등, 제외는 아님), 기관/외국인 **수급**, **공매도** 비중을 주입합니다.
- **catalyst(시간축)** — 미국은 다가오는 실적발표일(`실적 D-12`), 한국은 최근 공급계약(DART 단일판매·공급계약체결, `공급계약 6/1`)을 후보 카드에 붙입니다.
- **밸류에이션 기대치** — forward P/E를 성장률·현금흐름과 대조해 `정당화 가능`/`기대치 부담`/`과열`/`위험`/`저평가 후보`/`데이터 부족` 6단계로 판정합니다. trailing 가치평가·매력도 점수와는 **별개 렌즈**로 두고 카드 배지로만 교차표기합니다.
