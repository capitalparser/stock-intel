...
개인 투자 상황판은 투자 가설, 섹터, 거시 환경, 투자 성향을 같은 층위의 판단 관점으로 두고, 여러 관점이 동시에 가리키는 후보 종목을 점검합니다.
...
v1은 예시 자료로 판단 구조와 화면을 검증합니다. 실시간 가치평가·뉴스, 원문 수집, Telegram 전달은 후속 단계에서 같은 `dashboard` 구성 위에 붙입니다.
